...
const { executeCommand, formatBlock, insertHTML } = useCommands(restoreSelection, saveSelection, triggerInput)
...
const insertImage = (url: string, alt: string, align: string) => {
  let style = 'max-width: 100%; height: auto; border-radius: var(--vi-radius-md); box-shadow: var(--vi-shadow-sm); margin: 1rem 0;'
...
<template>
  <div class="vi-editor" :class="editorClass">
    ...
    <LinkModal v-if="openModalType === 'link'" @close="closeModal" @submit="insertLink" />
    <ImageModal v-if="openModalType === 'image'" @close="closeModal" @submit="insertImage" />
    <TableModal v-if="openModalType === 'table'" @close="closeModal" @submit="insertTable" />
    <FormulaModal v-if="openModalType === 'formula'" @close="closeModal" @submit="insertFormula" />
  </div>
</template>
...
.vi-editor-content :deep(p) { margin: 0 0 1.25rem 0; }
...
.vi-editor-content :deep(h1) { font-size: 2.25rem; }
.vi-editor-content :deep(h2) { font-size: 1.875rem; border-bottom: 1px solid var(--vi-border); padding-bottom: 0.5rem; }
.vi-editor-content :deep(h3) { font-size: 1.5rem; }
.vi-editor-content :deep(ul),
.vi-editor-content :deep(ol) { 
  padding-left: 1.5rem; 
  margin: 0 0 1.25rem 0; 
}
.vi-editor-content :deep(li) { margin-bottom: 0.5rem; }
